...
const char* objectFragmentSource = "#version 330 core \n"
    ...
    "}; \n"
    "struct Light { \n"
    "    vec3 direction; float cutOff; float outerCutOff; \n"
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUseProgram(objectProgram);
        glUniform3fv(glGetUniformLocation(objectProgram, "light.position"), 1, &(camera.Position[0]));
        glUniform3fv(glGetUniformLocation(objectProgram, "light.direction"), 1, &(camera.Front[0]));
        glUniform1f(glGetUniformLocation(objectProgram, "light.cutOff"), glm::cos(glm::radians(12.5f)));
        glUniform1f(glGetUniformLocation(objectProgram, "light.outerCutOff"), glm::cos(glm::radians(17.5f)));
        glUniform3fv(glGetUniformLocation(objectProgram, "viewPos"), 1, &(camera.Position[0]));

        // light properties
        glUniform1f(glGetUniformLocation(objectProgram, "light.constant"), 1.0f);
        glUniform1f(glGetUniformLocation(objectProgram, "light.linear"), 0.09f);
        glUniform1f(glGetUniformLocation(objectProgram, "light.quadratic"), 0.032f);
